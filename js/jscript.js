...
// Fungsi utama untuk memuat data produk dengan filter linkproduk
        ...
        productsRef.once('value', (snapshot) => {
            ...
            snapshot.forEach((sellerSnapshot) => {
                ...
                sellerSnapshot.child('products').forEach((productSnapshot) => {
                    ...
                    if (product.sku === sku) {
                        // CEK APAKAH PRODUK MEMILIKI LINKPRODUK
                        const linkprodukRef = database.ref(`sellers/${sellerId}/products/${productSnapshot.key}/linkproduk`);
                        linkprodukRef.once('value', (linkSnapshot) => {
                            const linkproduk = linkSnapshot.val();
                            
                            if (!linkproduk) {
                                // JIKA TIDAK ADA LINKPRODUK, SEMBUNYIKAN PRODUK
                                console.log('Produk tidak memiliki linkproduk, produk disembunyikan');
                                document.body.innerHTML = '<div style="text-align:center;padding:50px;color:red;">Produk tidak tersedia</div>';
                                return;
                            }
                            
                            productFound = true;
                            console.log("Product found with link:", linkproduk);
                            fetchSellerInfo(sellerId);

                            // Setup product link untuk gambar dan judul
                            const productLink = linkproduk;
                                ...
                                if (imageUrlsData) {
                                    ...
                                    sortedUrls.forEach((imageUrl, index) => {
                                        if (imageUrl) {
                                            ...
                                            img.loading = "lazy";
                                            img.style.cursor = 'pointer'; // Tambahkan cursor pointer
                                            // Tambahkan event click untuk gambar
                                            img.addEventListener('click', function() {
                                                window.location.href = productLink; // Buka di tab yang sama
                                            });
                                    ...
                                    if (product1234Slider.totalSlides > 0) {
                                        ...
                                        setupSliderImageClickEvents(productLink);
                            ...
                            // Set product name dengan event click
                            const productNameElement = document.getElementById('product-name');
                            productNameElement.textContent = product.name;
                            productNameElement.style.cursor = 'pointer'; // Tambahkan cursor pointer
                            productNameElement.title = 'Klik untuk melihat detail produk';
                            // Tambahkan event click untuk judul produk
                            ...
                            variationsRef.once('value', (variationsSnapshot) => {
                                ...
                                document.getElementById('beli-sekarang').onclick = () => {
                                    showQuantityPopup({...product, imageUrls: product1234Slider.images, linkproduk: productLink});
                                };
                            });
                        });
                    }
...
// Setup image click events untuk slider dengan linkproduk
function setupSliderImageClickEvents(productLink) {
    ...
    sliderImages.forEach((img, index) => {
        ...
        // Tambah event listener baru dengan productLink
        img.addEventListener('click', function() {
            if (productLink) {
                window.location.href = productLink; // Buka di tab yang sama
            }
...
function handleSliderImageClick(e, productLink) {
    if (productLink) {
        window.location.href = productLink; // Buka di tab yang sama
    } else {
...
(function() {
    ...
    JMDigitalCarousel.renderProducts = function() {
        ...
        digitalProducts.forEach(product => {
            ...
            // Product URL
            const productUrl = `https://www.jejakmufassir.my.id/p/belanja.html?produk=${product.sku}`;
            ...
            productElement.innerHTML = `
                ...
                <div class="jm-digital-carousel__product-details">
                    <div class="jm-digital-carousel__product-title">
                        <a href="${productUrl}" title="${product.name}">${truncatedName}</a>
                    </div>
        ...
        if (JMDigitalCarousel.targetSellerId && digitalProducts.length === 0) {
            ...
            noProductsMsg.textContent = 'Penjual ini tidak memiliki produk digital lainnya.';
