...
// Variabel untuk menyimpan linkproduk
                ...
                sellerSnapshot.child('products').forEach((productSnapshot) => {
                    ...
                    if (product.sku === sku) {
                        ...
                        linkprodukRef.once('value', (linkSnapshot) => {
                            ...
                            // Simpan linkproduk di variabel global
                            currentProductLink = linkproduk;
                            
                            productFound = true;
                            console.log("Product found with link:", currentProductLink);
                                        ...
                                        if (imageUrl) {
                                            ...
                                            img.addEventListener('click', function() {
                                                if (currentProductLink) {
                                                    window.location.href = currentProductLink; // Buka di tab yang sama
                                                }
                                            });
                            ...
                            variationsRef.once('value', (variationsSnapshot) => {
                                ...
                                document.getElementById('beli-sekarang').onclick = () => {
                                    showQuantityPopup({...product, imageUrls: product1234Slider.images, linkproduk: currentProductLink});
                                };
                            });
                        });
                    }
...
function showQuantityPopup(product) {
    ...
    createOrderBtn.addEventListener('click', () => {
        ...
        // Arahkan ke linkproduk jika ada, jika tidak ke halaman pesanan default
        if (product.linkproduk) {
            window.location.href = product.linkproduk;
        } else {
    ...
    JMDigitalCarousel.renderProducts = function() {
        ...
        (product.sellerId && product.id) {
                ...
                linkRef.once('value').then((snapshot) => {
                    ...
                    if (linkproduk) {
                        // Update link di elemen produk
                        const productLink = productElement.querySelector('a');
                        if (productLink) {
                            productLink.href = linkproduk;
                        }
                    }
                });
            }
